...
export function validateAadhaarChecksum(aadhaar: string): boolean {
  ...
  if (/^(\d)\1{11}$/.test(normalized)) return false;
...
export function getAadhaarValidationError(aadhaar: string): string | null {
  ...
  if (!/^\d+$/.test(normalized)) return 'Aadhaar must contain only digits';
  if (normalized.length !== 12) return 'Aadhaar must be exactly 12 digits';
  if (/^(\d)\1{11}$/.test(normalized)) return 'Invalid Aadhaar number';
  if (normalized[0] === '0' || normalized[0] === '1') return 'Aadhaar number cannot start with 0 or 1';
  if (!validateAadhaarChecksum(normalized)) return 'Invalid Aadhaar number. Checksum verification failed';
